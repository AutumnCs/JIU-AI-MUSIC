...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('community_posts_list_failed', cause);
    return error('internal', '社区暂时无法加载，请稍后再试', 500);
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('community_post_create_failed', cause);
    return error('internal', '发布失败，请稍后再试', 500);
  }
}

function containsBlockedText(value: string) { return ['色情', '暴恐', '自杀', '毒品', '赌博'].some((word) => value.includes(word)); }
